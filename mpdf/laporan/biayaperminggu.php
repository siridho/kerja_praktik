...
<h5 style='text-align:center;'>Biaya Alat</h5>";
...
$sqldaftaralat = "SELECT ada.kodealat as kode, a.nama as namaalat, SUM(ppl.volumepengukuran) as vol, SUM(ppl.total) as total FROM daftaralat da inner join alat_daftaralat ada on da.id=ada.iddaftaralat INNER JOIN alat a ON a.kode=ada.kodealat 
INNER JOIN pengukuranprogresslapangan ppl ON ppl.idkeperluan=a.kode
WHERE da.idproyek=".$id." and ada.iddaftaralat=(select max(dd.id) from daftaralat dd where dd.idproyek=".$id.") AND ppl.minggu=".$_GET['minggu'] ." GROUP BY ada.kodealat, a.nama";
$no=1;
$totalalat = 0;
...
$html.="<h5 style='text-align:center;'>Biaya material</h5>";
...
$sqldaftarmaterial = "SELECT ada.kode as kode, a.nama as namamaterial, SUM(ppl.volumepengukuran) as vol, SUM(ppl.total) as total FROM daftarmaterial da inner join material_daftarmaterial ada on da.id=ada.iddaftarmaterial INNER JOIN material a ON a.kode=ada.kode
INNER JOIN pengukuranprogresslapangan ppl ON ppl.idkeperluan=a.kode
WHERE da.idproyek=".$id." and ada.iddaftarmaterial=(select max(dd.id) from daftarmaterial dd where dd.idproyek=".$id.") AND ppl.minggu=".$_GET['minggu'] ." GROUP BY ada.kode, a.nama";
$no=1;
$totalmaterial=0;
$hasil=$a->query($sqldaftarmaterial);
while ($baris1=$hasil->fetch_assoc()) {
	if($baris1['vol']!=0){
	$html.=		"<tr>
			<td style='text-align:center;'>".$no."</td>
			<td style='text-align:center;'>".$baris1['kode']."</td>
			<td>".$baris1['namamaterial']."</td>
			<td style='text-align:center;'>".$baris1['vol']."</td>
			<td>Rp ".$baris1['total']."</td>
			</tr>";
		$no++;
		$totalmaterial+=$baris1['total'];
	}
}
	$html.='
	<tr>
	<td colspan="4" style="text-align:right;">Total </td>
	<td>Rp '.$totalmaterial.'</td>
</tr>
	</table>';
...
$html.="<br><br><h5 style='text-align:center;'>Akumulasi Biaya Alat Hingga Minggu ke-".$_GET['minggu']."</h5>";
...
$sqldaftaralat = "SELECT ada.kodealat as kode, a.nama as namaalat, SUM(ppl.volumepengukuran) as vol, SUM(ppl.total) as total FROM daftaralat da inner join alat_daftaralat ada on da.id=ada.iddaftaralat INNER JOIN alat a ON a.kode=ada.kodealat 
INNER JOIN pengukuranprogresslapangan ppl ON ppl.idkeperluan=a.kode
WHERE da.idproyek=".$id." and ada.iddaftaralat=(select max(dd.id) from daftaralat dd where dd.idproyek=".$id.") AND ppl.minggu BETWEEN 1 AND ".$_GET['minggu'] ." GROUP BY ada.kodealat, a.nama";
$no=1;
$totalalat = 0;
$hasil=$a->query($sqldaftaralat);
while ($baris1=$hasil->fetch_assoc()) {
...
$html.="<br><br><h5 style='text-align:center;'>Akumulasi Biaya Material Hingga Minggu ke-".$_GET['minggu']."</h5>";
...
$sqldaftarmaterial = "SELECT ada.kode as kode, a.nama as namamaterial, SUM(ppl.volumepengukuran) as vol, SUM(ppl.total) as total FROM daftarmaterial da inner join material_daftarmaterial ada on da.id=ada.iddaftarmaterial INNER JOIN material a ON a.kode=ada.kode
INNER JOIN pengukuranprogresslapangan ppl ON ppl.idkeperluan=a.kode
WHERE da.idproyek=".$id." and ada.iddaftarmaterial=(select max(dd.id) from daftarmaterial dd where dd.idproyek=".$id.") AND ppl.minggu  BETWEEN 1 AND ".$_GET['minggu'] ." GROUP BY ada.kode, a.nama";
$no=1;
$totalmaterial=0;
$hasil=$a->query($sqldaftarmaterial);
while ($baris1=$hasil->fetch_assoc()) {
