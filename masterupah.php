...
if(isset($_POST['btnmasuk'])){
    ...
    $sql=$a->prepare('insert into upah(kode,nama,satuan) values(?,?,?)');
        $sql->bind_param('sss',$kode,$nama,$satuan);
        $sql->execute();

    echo "<script>alert('data telah disimpan')</script>";

     header('Location:masterupah.php'); 
}
if(isset($_POST['btnedit'])){
    ...
    //$qu="update upah set nama=?, satuan=? where id=?";
        $sql=$a->prepare("update upah set nama=?, satuan=? where kode=?");
...
<body>
                                    ...
                                    <tbody>
                                        ...
                                        while ($baris=$hasil->fetch_assoc()) {
                                            ...
                                            <tr>
                                                ...
                                                <td id='namaupah<?php echo $baris['kode']; ?>'><?php echo $baris['nama'];?></td>
                                                <td id='satuanupah<?php echo $baris['kode']; ?>'><?php echo $baris['satuan'];?></td>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title" id="myModalLabel">Tambah upah</h4>
                                        </div>
                                        <div class="modal-body">
                                          <form method='post' action='masterupah.php'>
        <table>
            ...
            <tr class="form-group">
                ...
                <td><?php
                    ...
                    $sql='select max(kode) as kode from upah';
                    $hasil=$a->query($sql);
                    while ($baris=$hasil->fetch_assoc()) {
                        $kod=$baris['kode']+1;
                       // echo $kod;
                ...
                <td>
                    ...
                    <select name='satuan' class='form-control'>
                        <?php
                        $hasil=$a->query('select distinct satuan from upah');
                        while ($baris=$hasil->fetch_assoc()) {
    ...
    $(document).on("click", ".openedit", function () {
     ...
     var aaa='namaupah'+myBookId;
     $('.modal-body #nama').val($('#namaupah'+myBookId).text());
     $('#kod').val(myBookId);
     //document.getElementById('kod').value=='myBookId';
    $('.modal-body #satuan').val($('#satuanupah'+myBookId).text());
    //alert($('#kod').val());
    return false;
