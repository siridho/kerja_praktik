...
if(isset($_POST['btnmasuk'])){
    ...
    $sql=$a->prepare('insert into material(kode,nama,satuan,kelompok) values(?,?,?,?)');
        $sql->bind_param('ssss',$kode,$nama,$satuan,$kelompok);
        $sql->execute();

    echo "<script>alert('data telah disimpan')</script>";

     header('Location:mastermaterial.php'); 
}
if(isset($_POST['btnedit'])){
   ...
   $kelompok=$_POST['kelompok'];
    ...
    //$qu="update material set nama=?, satuan=? where id=?";
        $sql=$a->prepare("update material set nama=?, satuan=?, kelompok=? where kode=?");
        $sql->bind_param("ssss", $nama,$satuan,$kelompok,$kode);
        $sql->execute();
    ...
    $_SESSION['halaman']='mastermaterial';
...
<body>
                                    ...
                                    <tbody>
                                        ...
                                        while ($baris=$hasil->fetch_assoc()) {
                                            ...
                                            <tr>
                                                ...
                                                <td id='namamaterial<?php echo $baris['kode']; ?>'><?php echo $baris['nama'];?></td>
                                                <td id='satuanmaterial<?php echo $baris['kode']; ?>'><?php echo $baris['satuan'];?></td>
                                                <td id='kelompokmaterial<?php echo $baris['kode']; ?>'><?php echo $baris['kelompok'];?></td>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title" id="myModalLabel">Tambah material</h4>
                                        </div>
                                        <div class="modal-body">
                                          <form method='post' action='mastermaterial.php'>
        <table>
            ...
            <tr class="form-group">
                ...
                <td><?php
                    ...
                    $sql='select max(kode) as kode from material';
                    $hasil=$a->query($sql);
                    while ($baris=$hasil->fetch_assoc()) {
                        $kod=$baris['kode']+1;
                       // echo $kod;
                ...
                <td>
                    ...
                    <select name='satuan' class='form-control'>
                        <?php
                        $hasil=$a->query('select distinct satuan from material');
                        while ($baris=$hasil->fetch_assoc()) {
            ...
            <tr>
                ...
                <td>
                    ...
                    <select name='kelompok' class='form-control'>
                        <?php
                        $hasil=$a->query('select distinct kelompok from material order by kelompok asc');
                        while ($baris=$hasil->fetch_assoc()) {
                             echo "<option value='".$baris['kelompok']."'>".$baris['kelompok']."</option>";
                         } 
                        ?>
    ...
    $(document).on("click", ".openedit", function () {
     ...
     var aaa='namamaterial'+myBookId;
     $('.modal-body #nama').val($('#namamaterial'+myBookId).text());
     $('#kod').val(myBookId);
     //document.getElementById('kod').value=='myBookId';
    $('.modal-body #satuan').val($('#satuanmaterial'+myBookId).text());
    $('modal-body #kelompok').val($('#kelompokmaterial'+myBookId).text());
    //alert($('#kod').val());
    return false;
