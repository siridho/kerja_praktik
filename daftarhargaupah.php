...
if(isset($_POST['simpan'])){
    ...
    $sql='select count(id) as jum from daftarupah where nama like"%'.$_POST['namaproyek'].'%"';
    $hasil=$a->query($sql);
    while ($baris=$hasil->fetch_assoc()) {
        $jum=$baris['jum'];
    }
    ...
    if($jum == 0){
        ...
        while ($baris = $hasil->fetch_assoc()) {
            $tam=$baris['id'];
            $ins=$a->prepare('INSERT into daftarupah(`nama`,`idproyek`) values(?,?)');
        ...
        $sql='select max(id) as id from proyek where nama="'.$_SESSION['namaproyek'].'"';
        $hasil=$a->query($sql);
        while ($baris=$hasil->fetch_assoc()) {
            $tam=$baris['id'];
            $ins=$a->prepare('INSERT into daftarupah(`nama`,`idproyek`) values(?,?)');
    ...
    $sql = 'select id from daftarupah where nama="'.$_SESSION['namaproyek'].'"';
    $hasil = $a->query($sql);
    while ($baris = $hasil->fetch_assoc()) {
        $iddaftarupah = $baris['id'];
    }
    ...
    for($i=0 ; $i<$itung ; $i++){
        ...
        if(isset($_POST[$nam])){
            ...
            $ins=$a->prepare('INSERT INTO `upah_daftarupah`(`harga`, `kodeupah`, `iddaftarupah`) VALUES (?,?,?)');
            $ins->bind_param('isi',$harga,$kode,$iddaftarupah);
            $ins->execute();
        }

    }

    header('Location:daftarhargamaterial.php');
}

    $_SESSION['halaman']='daftarhargaupah';
    $_SESSION['user']='';
...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper" >
         <form method='post' action='daftarhargaupah.php'>
            <div id="page-inner">
             <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-header">
                            Buat Daftar Harga - Upah
                        </h1>
                        <ol class="breadcrumb">
                          <li>Daftar Harga Alat</li>
                          <li class="active" style="color:red;">Daftar Harga Upah</li>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <tbody>
                                        ...
                                        while ($baris=$hasil->fetch_assoc()) {
                                            ...
                                            <tr>
                                                <td>
                                                    <input type='checkbox' class='form-control checkupah' name='cek<?php echo $baris['kode']; ?>' checked value='<?php echo $baris['kode'];?>'>
                                                 </td>
                            ...
                            <input type='hidden' name='kode<?php echo $itung; ?>' value='<?php echo $baris['kode']; ?>'>
                                ...
                                <input type='text' required class='form-control hargaupah' name='hargaupah<?php echo $itung; ?>' id='<?php echo $baris['kode']; ?>' value='50000'>
    ...
    <script type="text/javascript">
        ...
        $(document).on('change', '.checkupah', function() {
            if(!this.checked){
                //  alert($(this).val());
                var k=$(this).val();
               // var idd='hargaupah'+k;
